...
const EditFlowerDialog = ({
  ...
  return <MobileDialog open={open} onOpenChange={onOpenChange}>
      <MobileDialogContent>
        <MobileDialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex-1 p-6">
          <div className="space-y-4">
            <Input type="number" value={quantity} onChange={e => setQuantity(e.target.value)} min="0" placeholder="Cantitate nouă" />
          </div>
        </form>
        <MobileDialogFooter className="p-6 border-t flex justify-end fixed bottom-0 w-full bg-white pb-safe">
          <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="my-[12px]">
            Anulează
          </Button>
